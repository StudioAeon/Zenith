...
// #include "Zenith/ImGui/ImGui.hpp"
...
namespace Zenith {
	...
	void VulkanShader::RT_Reload(const bool forceCompile)
	{
		...
		{
			ZN_CORE_FATAL("Failed to recompile shader!");
		}
#endif
	}
	...
	void VulkanShader::CreateDescriptors()
	{
		...
		for (uint32_t set = 0; set < m_ReflectionData.ShaderDescriptorSets.size(); set++)
		{
			...
			for (auto& [bindingAndSet, imageSampler] : shaderDescriptorSet.StorageImages)
			{
				...
				ZN_CORE_ASSERT(shaderDescriptorSet.ImageSamplers.find(binding) == shaderDescriptorSet.ImageSamplers.end(), "Binding is already present!");
				ZN_CORE_ASSERT(shaderDescriptorSet.SeparateTextures.find(binding) == shaderDescriptorSet.SeparateTextures.end(), "Binding is already present!");
				ZN_CORE_ASSERT(shaderDescriptorSet.SeparateSamplers.find(binding) == shaderDescriptorSet.SeparateSamplers.end(), "Binding is already present!");
			...
			ZN_CORE_TRACE_TAG("Renderer", "Creating descriptor set {0} with {1} ubo's, {2} ssbo's, {3} samplers, {4} separate textures, {5} separate samplers and {6} storage images", set,
	...
	{
		ZN_CORE_ASSERT(set < m_DescriptorSetLayouts.size());
	...
	VulkanShader::ShaderMaterialDescriptorSet VulkanShader::CreateDescriptorSets(uint32_t set)
	{
		...
		ZN_CORE_ASSERT(m_TypeCounts.find(set) != m_TypeCounts.end());
	...
	VulkanShader::ShaderMaterialDescriptorSet VulkanShader::CreateDescriptorSets(uint32_t set, uint32_t numberOfSets)
	{
		...
		ZN_CORE_ASSERT(poolSizes.find(set) != poolSizes.end());
	...
	{
		ZN_CORE_ASSERT(set < m_ReflectionData.ShaderDescriptorSets.size());
		ZN_CORE_ASSERT(m_ReflectionData.ShaderDescriptorSets[set]);
		...
		{
			ZN_CORE_WARN_TAG("Renderer", "Shader {0} does not contain requested descriptor set {1}", m_Name, name);
			return nullptr;
		}
