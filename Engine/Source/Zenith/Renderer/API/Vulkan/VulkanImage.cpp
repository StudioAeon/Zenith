...
namespace Zenith {
	...
	{
		ZN_CORE_VERIFY(m_Specification.Width > 0 && m_Specification.Height > 0);
	}
	...
	{
		ZN_CORE_ASSERT(m_Specification.Layers > 1);
	...
	void VulkanImage2D::RT_CreatePerSpecificLayerImageViews(const std::vector<uint32_t>& layerIndices)
	{
		...
		//ZN_CORE_ASSERT(m_PerLayerImageViews.size() == m_Specification.Layers);
	...
	void VulkanImage2D::UpdateDescriptor()
	{
		...
		//ZN_CORE_WARN_TAG("Renderer", "VulkanImage2D::UpdateDescriptor to ImageView = {0}", (const void*)m_Info.ImageView);
	}
	...
	{
		ZN_CORE_VERIFY(m_Specification.Transfer, "Image must be created with ImageSpecification::Transfer enabled!");
	...
	void VulkanImage2D::CopyToHostBuffer(Buffer& buffer) const
	{
		...
		ZN_CORE_WARN("MemReq = {} ({})", memReqs.size, memReqs.alignment);
		ZN_CORE_WARN("Expected size = {}", bufferSize);
#endif
