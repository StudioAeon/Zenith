...
#include "Zenith/ImGui/ImGuiFonts.hpp"
...
namespace Zenith {
	...
	VulkanImGuiLayer::VulkanImGuiLayer(ApplicationContext& context) : ImGuiLayer(context)
	{
	}
	...
	void VulkanImGuiLayer::OnAttach()
	{
		...
		// Configure Fonts
		{
			...
			robotoSmall.FilePath = "Resources/Fonts/Roboto/Roboto-Medium.ttf";
			robotoSmall.Size = 12.0f;
			UI::Fonts::Add(robotoSmall);

			UI::FontConfiguration robotoExtraSmall;
			robotoExtraSmall.FontName = "ExtraSmall";
			robotoExtraSmall.FilePath = "Resources/Fonts/Roboto/Roboto-Medium.ttf";
		...
		//{
			// Use ApplicationContext instead of Application::Get()
			Window& window = instance->GetContext().GetWindow();
			...
			// Setup Platform/Renderer bindings - Use SDL3 instead of GLFW
			...
			VulkanSwapChain& swapChain = window.GetSwapChain();
			...
			for (uint32_t i = 0; i < framesInFlight; i++)
				s_ImGuiCommandBuffers[i] = VulkanContext::GetCurrentDevice()->CreateSecondaryCommandBuffer("ImGuiSecondaryCommandBuffer");
		//});
	}
	...
	void VulkanImGuiLayer::End()
	{
		...
		VulkanSwapChain& swapChain = GetContext().GetWindow().GetSwapChain();
