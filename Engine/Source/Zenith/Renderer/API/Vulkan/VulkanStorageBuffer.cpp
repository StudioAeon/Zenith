...
#define NO_STAGING 1
	...
	{
		ZN_PROFILE_FUNC();
		ZN_SCOPE_PERF("VulkanStorageBuffer::RT_SetData");

		// Cannot call SetData if GPU only
		ZN_CORE_VERIFY(!m_Specification.GPUOnly);
		...
		{
			ZN_SCOPE_PERF("VulkanStorageBuffer::RT_SetData - MemoryMap");
		...
		{
			ZN_SCOPE_PERF("VulkanStorageBuffer::RT_SetData - CommandBuffer");
