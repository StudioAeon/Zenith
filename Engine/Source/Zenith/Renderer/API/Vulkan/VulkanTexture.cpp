...
namespace Zenith {
	...
	namespace Utils {
		...
		static VkSamplerAddressMode VulkanSamplerWrap(TextureWrap wrap)
		{
			...
			ZN_CORE_ASSERT(false, "Unknown wrap mode");
		...
		static VkFilter VulkanSamplerFilter(TextureFilter filter)
		{
			...
			ZN_CORE_ASSERT(false, "Unknown filter");
			return (VkFilter)0;
		}
	...
	void VulkanTexture2D::CreateFromFile(const TextureSpecification& specification, const std::filesystem::path& filepath)
	{
		...
		if (!m_ImageData)
		{
			...
			ZN_CORE_ERROR("Failed to load texture from file: {}", filepath);
		...
		ZN_CORE_ASSERT(m_Specification.Format != ImageFormat::None);
	...
	void VulkanTexture2D::SetData(Buffer buffer)
	{
		...
		ZN_CORE_ASSERT(m_ImageData.Data);
	...
	void VulkanTextureCube::Release()
	{
		...
		{
			ZN_CORE_TRACE_TAG("Renderer", "Destroying VulkanTextureCube");
	...
	{
		// ZN_CORE_VERIFY(buffer.Size == m_GPUAllocationSize);
