...
#include "API/Vulkan/VulkanShader.hpp"
#include "API/Vulkan/VulkanContext.hpp"
#include "API/Vulkan/VulkanFramebuffer.hpp"
#include "API/Vulkan/VulkanUniformBuffer.hpp"
...
namespace Zenith {
	...
	namespace Utils {
		static VkPrimitiveTopology GetVulkanTopology(PrimitiveTopology topology)
		{
			...
			ZN_CORE_ASSERT(false, "Unknown toplogy");
		...
		static VkCompareOp GetVulkanCompareOperator(const DepthCompareOperator compareOp)
		{
			...
			ZN_CORE_ASSERT(false, "Unknown Operator");
	...
	void Pipeline::Invalidate()
	{
		...
		{
			// ZN_CORE_WARN("[Pipeline] Creating pipeline {0}", instance->m_Specification.DebugName);
	...
	{
		return Ref<Pipeline>::Create(spec);
	}
